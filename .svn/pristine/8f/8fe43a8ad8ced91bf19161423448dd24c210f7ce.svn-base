 ...
 ;(function($,window,document){
 	...
 	DialogBox.prototype = {	
 		...
 		create: function(element){
 			...
 			if(!hasBtn){
 				...
 				if(confirmValue && cancelValue){
 					...
 					dialogHTML[1] = '<div class="dialog-btn"><span class="dialog-btn-confirm">确定</span><span class="dialog-btn-cancel">取消</span></div></div></section>';
 				}
 			}
 		...
 		trigger: function(element,event){
 			...
 			$('.dialog-box-close,.dialog-btn-cancel,.dialog-btn-confirm').bind('click',function(){
 				that.hide($this);
 			});
 			...
 			if($.isFunction(that.settings.confirm)){
 				$('.dialog-btn-confirm').bind('click',function(){
